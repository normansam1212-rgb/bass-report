...
var ALL_LAKES = [
  ...
  { name:"Lake J.B. Thomas",        lat:32.598719,  lon:-101.212962, type:"Reservoir", region:"Snyder"        },

  { name:"Lake Sweetwater",         lat:32.422217,  lon:-100.309401, type:"Reservoir", region:"Sweetwater"    },

  // NORTH CENTRAL / ABILENE

  { name:"Lake Stamford",           lat:33.053132,  lon:-99.610694,  type:"Reservoir", region:"Haskell"       },

  { name:"Lake Fort Phantom Hill",  lat:32.5755,  lon:-99.6868,  type:"Reservoir", region:"Abilene"       },

  { name:"Lake Kirby",              lat:32.3712,  lon:-99.7325,  type:"Reservoir", region:"Abilene"       },
  ...
  { name:"Possum Kingdom Lake",     lat:32.912062,  lon:-98.505152,  type:"Reservoir", region:"Mineral Wells" },

  { name:"Lake Palo Pinto",         lat:32.6425,  lon:-98.3006,  type:"Reservoir", region:"Palo Pinto"    },
  ...
  { name:"Millers Creek Reservoir", lat:33.399556,  lon:-99.391292,  type:"Reservoir", region:"Munday"        },

  { name:"Bluestem Lake",           lat:32.5283,  lon:-99.2967,  type:"Reservoir", region:"Albany"        },

  { name:"Lake Throckmorton",       lat:33.164869,  lon:-99.194581,  type:"Reservoir", region:"Throckmorton"  },

  { name:"Lake Winters",            lat:31.963053,  lon:-99.86961,  type:"Reservoir", region:"Winters"       },
  ...
  { name:"Lake Diversion",          lat:33.792505,  lon:-98.975785,  type:"Reservoir", region:"Wichita Falls" },

  { name:"Lake Amon G. Carter",     lat:33.472311,  lon:-97.888252,  type:"Reservoir", region:"Bowie"         },

  { name:"Nocona Lake",             lat:33.8579,  lon:-97.6552,  type:"Reservoir", region:"Nocona"        },

  { name:"Lake Ray Roberts",        lat:33.3348,  lon:-97.0285,  type:"Reservoir", region:"Denton"        },
  ...
  { name:"Benbrook Lake",           lat:32.625439,  lon:-97.465102,  type:"Reservoir", region:"Fort Worth"    },
  ...
  { name:"Joe Pool Lake",           lat:32.59038,  lon:-97.021852,  type:"Reservoir", region:"Dallas"        },

  { name:"Lake Arlington",          lat:32.698208,  lon:-97.212824,  type:"Reservoir", region:"Arlington"     },

  { name:"Mountain Creek Lake",     lat:32.715815,  lon:-96.960329,  type:"Reservoir", region:"Dallas"        },
  ...
  { name:"Lake Fork",               lat:32.886004,  lon:-95.614822,  type:"Reservoir", region:"Quitman"       },

  { name:"Lake Bob Sandlin",        lat:33.054806,  lon:-95.079124,  type:"Reservoir", region:"Mt Pleasant"   },

  { name:"Lake Cypress Springs",    lat:33.061218,  lon:-95.198515,  type:"Reservoir", region:"Mt Vernon"     },
  ...
  { name:"Lake Jacksonville",       lat:31.922489,  lon:-95.293792,  type:"Reservoir", region:"Jacksonville"  },

  { name:"Lake Athens",             lat:32.215286,  lon:-95.752341,  type:"Reservoir", region:"Athens"        },
  ...
  { name:"Lake Bryan",              lat:30.712427,  lon:-96.466238,  type:"Reservoir", region:"Bryan"         },

  { name:"Gibbons Creek Reservoir", lat:30.632449,  lon:-96.06242,  type:"Reservoir", region:"Navasota"      },
  ...
  { name:"Inks Lake",               lat:30.743593,  lon:-98.388281,  type:"Reservoir", region:"Burnet"        },
  ...
  { name:"Lake Placid",             lat:29.5598,  lon:-98.0101,  type:"Reservoir", region:"Seguin"        },
  ...
  { name:"Falcon Reservoir",        lat:26.7183,  lon:-99.236,  type:"Reservoir", region:"Laredo"        },

  { name:"Lake Casa Blanca",        lat:27.551073,  lon:-99.431827,  type:"Reservoir", region:"Laredo"        },

  { name:"Amistad Reservoir",       lat:29.477393,  lon:-101.035842, type:"Reservoir", region:"Del Rio"       },
...
var MAX_SHOWN = 15;
var _pinnedLakeName = null; // lake name typed directly into search — pinned to top
...
// Quick-pick a famous lake from the empty-state chips
function quickPick(name, lat, lon) {
  _pinnedLakeName = name;
...
function selectLocation(lat, lon, name) {
  ...
  // Check if the search term matches a named lake in our database — pin it to the top
  _pinnedLakeName = null;
  var _nameLower = name.toLowerCase().trim();
  for (var _pi = 0; _pi < ALL_LAKES.length; _pi++) {
    if (ALL_LAKES[_pi].name.toLowerCase() === _nameLower) {
      _pinnedLakeName = ALL_LAKES[_pi].name;
      // Use the lake's own coords so distance sorting puts it first
      userLat = ALL_LAKES[_pi].lat;
      userLon = ALL_LAKES[_pi].lon;
      break;
    }
  }

  // Save for next visit
  try { localStorage.setItem("bass-loc", JSON.stringify({lat:userLat,lon:userLon,label:name})); } catch(e){}
...
function fetchSun(lat, lon) {
  ...
  return fetch("https://api.sunrisesunset.io/json?lat="+lat+"&lng="+lon)
    ...
    .catch(function() {
      // Fallback: derive sunrise/sunset from Open-Meteo daily data
      return fetch("https://api.open-meteo.com/v1/forecast?latitude="+lat+"&longitude="+lon
        +"&daily=sunrise,sunset&timezone=auto&forecast_days=1")
        .then(function(r){return r.json();})
        .then(function(d){
          var sr = d.daily&&d.daily.sunrise&&d.daily.sunrise[0];
          var ss = d.daily&&d.daily.sunset &&d.daily.sunset[0];
          // Open-Meteo returns ISO strings like "2024-06-01T06:23"
          // parseSunTime already handles ISO format (indexOf("T") check)
          return { sunrise: sr||"06:30:00 AM", sunset: ss||"08:15:00 PM" };
        });
    });
...
function renderHero(top, baits, moon) {
  ...
  el.innerHTML=
    ...
    '<div class="hero-lbl"><span style="width:18px;height:18px;display:inline-block;vertical-align:middle">'+IC.trophy+'</span> '+(_pinnedLakeName && top.name===_pinnedLakeName ? 'Lake Report' : 'Best Bet Today')+'</div>'+
...
function renderLakes(weather, trend, sun, moon) {
  ...
  // If user searched a specific lake by name, guarantee it's included + shown first
  var pinnedIdx = -1;
  if (_pinnedLakeName) {
    for (var _pj = 0; _pj < scored.length; _pj++) {
      if (scored[_pj].name === _pinnedLakeName) { pinnedIdx = _pj; break; }
    }
  }
  ...
  // Ensure pinned lake is in the list even if it's beyond the nearest-N window
  if (pinnedIdx >= MAX_SHOWN) {
    nearest.push(scored[pinnedIdx]);
  }

  var maxDist=nearest[nearest.length-1].dist;

  nearest.sort(function(a,b){
    // Pinned lake always first
    if (_pinnedLakeName) {
      if (a.name === _pinnedLakeName) return -1;
      if (b.name === _pinnedLakeName) return  1;
    }
  ...
  // Seasonal pattern callout — plain-English tip above the cards
  var _month = new Date().getMonth()+1;
  var _seasonTip = (function(m){
    if(m>=3&&m<=4)  return "🌱 Spring pre-spawn — fish are moving shallow. Target points, creek mouths, and 45° banks with jerkbaits and swimbaits.";
    if(m===5)       return "🥚 Spawn is on — look for beds in 2–8 ft on firm bottom. Sight-fish with Senkos or creature baits. Handle carefully.";
    if(m===6)       return "☀️ Post-spawn — females recovering deep, males still on beds. Deep points, brush piles. Crankbaits and Carolina rigs shine.";
    if(m>=7&&m<=8)  return "🔥 Summer heat — fish deep structure at dawn/dusk. Topwater at first light, then drop shot or football jig by 9am.";
    if(m===9)       return "🍂 Early fall — baitfish moving shallow. Bass following. Great time for lipless cranks and spinnerbaits on flats.";
    if(m>=10&&m<=11)return "🍁 Fall feeding binge — bass gorging pre-winter. Reaction baits, crankbaits, swim jigs. Some of the best action of the year.";
    if(m===12)      return "❄️ Early winter — fish slowing down, moving deeper. Slow roll a swimbait or drag a jig on main-lake points.";
    return "🧊 Deep winter — bass in deepest available water. Jigging spoon, blade bait or finesse drop shot. Slow is key.";
  })(_month);
  var _calloutEl = document.getElementById("seasonCallout");
  if(_calloutEl) { _calloutEl.textContent = _seasonTip; _calloutEl.style.display="block"; }
...
function renderFilteredCards(nearest, weather, trend, moon) {
  ...
  sorted.forEach(function(lake, i) {
    ...
    // Collapse breakdown: top-3 positive contributions + 1 worst negative, then expander
    var _pos = lake.bd.filter(function(b){return b.m>0;}).sort(function(a,b){return b.m-a.m;});
    var _neg = lake.bd.filter(function(b){return b.m<0;}).sort(function(a,b){return a.m-b.m;});
    var _shown = _pos.slice(0,3).concat(_neg.slice(0,1));
    var _hidden = lake.bd.filter(function(b){ return _shown.indexOf(b)===-1; });
    var tags = _shown.map(function(b){
      var cls=b.m>0?"pos":b.m<0?"neg":"neu";
      return '<span class="stag '+cls+'">'+b.l+' '+(b.m>0?"+":"")+b.m+'</span>';
    }).join("");
    if (_hidden.length) {
      tags += '<span class="stag neu stag-more" onclick="event.stopPropagation();this.parentNode.innerHTML+=\''+
        _hidden.map(function(b){var cls=b.m>0?\'pos\':(b.m<0?\'neg\':\'neu\');return \'<span class=\"stag \'+cls+\'\">\'+b.l+\' \'+(b.m>0?\'+\':\'\')+b.m+\'</span>\';}).join(\'\')
        +'\'+(this.style.display=\'none\')">+'+_hidden.length+' more</span>';
    }
        ...
        '<div class="card-body">'+
          ...
          '<div class="card-top">'+
            ...
            '<div class="card-info">'+
              ...
              '<div class="card-meta">'+distLabel+' · '+lake.type+' · '+lake.region+' · '+IC.water+' ~'+lake.wt+'°F est.</div>'+
...
function _makePinIcon(score, hex) {
  // Clean circle badge — iconAnchor at exact center [half, half]
  // so Leaflet places the center of the badge precisely on the lat/lon.
  // Sizes: hot=34, ok=28, cold=22px diameter (visual hierarchy)
  var s = score >= 7 ? 34 : score >= 5 ? 28 : 22;
  var half = s / 2;
  var cls = score >= 7 ? "msp-hot" : score >= 5 ? "msp-ok" : "msp-cold";
  var fs = score >= 7 ? Math.round(s * 0.42) : Math.round(s * 0.40);
  var html =
    '<div class="msp-badge ' + cls + '" style="' +
  ...
  return L.divIcon({
    ...
    html: html,
    iconSize: [s, s],
    iconAnchor: [half, half],          // center of circle == lat/lon point
    popupAnchor: [0, -half - 4]        // popup sits just above the badge
  });
}
...
function showLoading() {
  ...
  var _sc=document.getElementById("seasonCallout"); if(_sc)_sc.style.display="none";
  ...
  if(!userLat||!userLon){
      ...
      document.getElementById("lakesList").innerHTML=
        '<div class="empty-state">'+
          '<div class="es-headline">Real-time bass conditions for every Texas lake</div>'+
          '<div class="es-sub">Weather · Baro · Moon · Water Temp · Bait picks — updated live</div>'+
          '<div class="es-chips-label">Quick-pick a lake to start:</div>'+
          '<div class="es-chips">'+
            '<button class="es-chip" onclick="quickPick('Lake Fork',32.89,-95.57)">🏆 Lake Fork</button>'+
            '<button class="es-chip" onclick="quickPick('Sam Rayburn Reservoir',31.07,-94.11)">🎣 Sam Rayburn</button>'+
            '<button class="es-chip" onclick="quickPick('Toledo Bend Reservoir',31.18,-93.57)">Toledo Bend</button>'+
            '<button class="es-chip" onclick="quickPick('Lake Conroe',30.33,-95.42)">Lake Conroe</button>'+
            '<button class="es-chip" onclick="quickPick('Lake Travis',30.38,-98.00)">Lake Travis</button>'+
            '<button class="es-chip" onclick="quickPick('Falcon Reservoir',26.57,-99.15)">Falcon Res.</button>'+
            '<button class="es-chip" onclick="quickPick('Choke Canyon Reservoir',28.47,-98.25)">Choke Canyon</button>'+
            '<button class="es-chip" onclick="quickPick('Lake Texoma',33.80,-96.60)">Lake Texoma</button>'+
          '</div>'+
          '<div class="es-or">— or search any city / lake above —</div>'+
        '</div>';
